...
TEST(FormatterTestSolitary, BasicFormatterIncludesTimestampLevelAndMessage)
{
    ...
    auto containsDate = [](const std::string &s) {
        ...
        { // [YYYY-MM-DD] = 10 chars
            if (std::isdigit(s[i + 1]) && std::isdigit(s[i + 2]) && std::isdigit(s[i + 3]) &&
                std::isdigit(s[i + 4]) && s[i + 5] == '-' && std::isdigit(s[i + 6]) &&
                std::isdigit(s[i + 7]) && s[i + 8] == '-' && std::isdigit(s[i + 9]))
                return true;
        }
        return false;
    };
    ...
    auto containsTime = [](const std::string &s) {
        ...
        { // HH:MM:SS = 8 chars
            if (std::isdigit(s[i]) && std::isdigit(s[i + 1]) && s[i + 2] == ':' &&
                std::isdigit(s[i + 3]) && std::isdigit(s[i + 4]) && s[i + 5] == ':' &&
